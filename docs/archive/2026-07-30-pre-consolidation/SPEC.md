# SAY TO THE KING, MY LORD — ARCHIVED PRE-CONSOLIDATION SPECIFICATION
...
> Historical record only. Repository-root `SPEC.md` supersedes this document.
...
### Law 8 — The required language model never decides the world

A lightweight local model is part of the court's communication
infrastructure. It voices people from their permitted Belief, interprets and
drafts letters against a closed action grammar, translates, and summarizes
selected records. It may not invent facts, calculate outcomes, choose player
or NPC policy, read hidden World state, or mutate state. Structured meanings
remain authoritative; accepted generated text is stored so replay never reruns
the model.

The supported baseline is a small local model, currently
`qwen3:4b-instruct`. Missing
model service or weights is a setup failure with a direct repair instruction,
not a second, fully featured “AI Off” mode.
...
- the Hall, combined Scribes' Room, Counsel, and retrieval-grounded Help;
...
- Scribes' Room selection must not hide a newly read tablet before its body is
  shown.
- Incoming correspondence needs Read, Answer/Respond, Delegate, Compare, and
  Archive paths, plus Outbox/in-transit state.
- Archive results must open within the Scribes' Room, with deliberate pinning
  available for cross-window comparison.
- Storehouse must expose the complete stores list, melt ledger, bronze in
  circulation, labour roll, and land accounts without clipped rows.
...
- Model work is asynchronous, bounded, grounded, and recoverable without ever
  granting the model simulation authority.
...
- Help is immediate, deterministic, free, searchable, and generated from the
  live control/action contract. Counsel and correspondence are required
  model-backed human-language surfaces; exact structured terms, evidence, and
  costs remain visible beside their prose.
- Model work is cancellable, request-scoped, unable to steal focus, unable to
  change deterministic capability, and cached or stored once accepted.
...
- required-model startup, grounded output, timeout recovery, and stored-text
  replay;
