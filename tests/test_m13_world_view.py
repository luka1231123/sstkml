...
def test_the_route_tablet_scrolls_and_every_place_stays_clickable() -> None:
    ...
    # crowded out is named under the map and is still clickable there: a place
    # is never silently dropped, only ever moved off the window.
    text = plain_text(first)
    assert "elsewhere on the map" in text
