...
def test_legacy_diplomacy_phrases_redirect_to_world_and_desk(words):
    belief = _belief()
    ...
    class ModelMustNotBeCalled:
        def call(self, *_args, **_kwargs):
            raise AssertionError("legacy diplomacy should be caught locally")
...
@pytest.mark.parametrize("verb", ("SEND_GIFT", "MARRY_ABROAD"))
def test_model_action_decoder_has_no_legacy_diplomacy_verb(verb):
...
def test_terminal_mutation_funnel_refuses_legacy_diplomacy(action):
...
def test_legacy_log_records_still_decode_and_reach_reduce(monkeypatch):
    marker = object()
    seen = []
    ...
    monkeypatch.setattr(relations, "send_gift", replay_gift)
    monkeypatch.setattr(house, "marry_abroad", replay_marriage)
